...
void PageLib::create()
{
    ...
    vector<string> web_page_path = _dir_scanner._files;
    ...
    for(size_t i = 0; i < _web_pages.size(); i++)
    {
        ...
        _offset_lib[i + 1] = {cur_page_begin_pos, cur_page_length - 1}; 
...
void PageLib::processWebPage(const string file_path)
{
    XMLDocument doc;
...
void PageLib::getRssItem(XMLElement * item_element, string & title, string & link, string & content)
{
...
void PageLib::deleteHTMLTag(string & content)
{
...
string PageLib::generateWebPage(string & title, string & link, string & content)
{
    ostringstream oss;
