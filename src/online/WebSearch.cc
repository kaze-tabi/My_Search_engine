...
WebSearch::WebSearch()
{
    _p_split_tool = WebPageQuery::getInstance()->getSpliTool();
}
...
void WebSearch::generateCandidateWebPageSet(set<int> & web_page_set, vector<string> split_res)
{
    ...
    auto invert_index_lib = WebPageQuery::getInstance()->getInvertIndexLib();
...
void WebSearch::generateWebPage(vector<int> & most_similar_doc_id_vec)
{
    ...
    auto offset_lib_ves = WebPageQuery::getInstance()->getOffsetLib();
