...
LRUcache * CacheManager::getThreadCache(pid_t thread_id)
{
    ...
    {
        _tid_cache_map[thread_id] = new DoubleLRU();
...
void CacheManager::updateAllThreadCache()
{
    ...
    mutex & main_cache_mutex =  umit->second->getLRULock();
    ...
    {
        mutex & thread_cache_mutex = umit->second->getLRULock();
...
void CacheManager::updateAllThreadDoubleCache()
{
    ...
    LRUcache * main_cache = umit->second;

    // 2. 合并从节点的内容
    SubLRUCache temp_cache;

    while (umit != _tid_cache_map.end())
    {
        lock_guard<mutex> cur_cache_syn_lock(umit->second->getLRULock());
    ...
    for(auto it = temp_cache.getLRUCacheContent().begin(); it != temp_cache.getLRUCacheContent().end(); ++it)
    {
        std::cout << " " << it->first;
    }
    ...
    auto temp_cache_content = temp_cache.getLRUCacheContent();
    umit = _tid_cache_map.begin();
    while(umit != _tid_cache_map.end())
    {
        std::cout << "\t\tthread[ " << umit->first << " ] -> Update Cache Success...\n";
